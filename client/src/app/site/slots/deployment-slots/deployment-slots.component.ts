...
export class DeploymentSlotsComponent extends FeatureComponent<TreeViewInfo<SiteData>> implements OnDestroy {
  ...
  private _siteResourceId: ResourceId;
  ...
  protected setup(inputEvents: Observable<TreeViewInfo<SiteData>>) {
    return inputEvents
      ...
      .switchMap(viewInfo => {
        ...
        this._siteResourceId = siteDescriptor.getSiteOnlyResourceId().toLowerCase();

        return Observable.zip(
          this._siteService.getSite(this._siteResourceId, this._refreshing),
          this._siteService.getSlots(this._siteResourceId, this._refreshing),
          this._siteService.getSiteConfig(this._siteResourceId, this._refreshing)
        );
      })
      .switchMap(r => {
  ...
  private _setupSlotNewMessageSubscription() {
    this._broadcastService
      ...
      .filter(m => {
        ...
        return resourceId === this.resourceId.toLowerCase() || resourceId === this._siteResourceId.toLowerCase();
      })
      .subscribe(message => {
